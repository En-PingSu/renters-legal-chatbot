...
\begin{table}[t]
  \caption{Self-evaluation bias: self-judging inflates faithfulness scores for all models, with Llama showing the largest bias (+0.429). Using an independent judge family (Claude Sonnet 4) eliminates this artifact.}
  \label{tab:self_eval}
  \centering
  \begin{tabular}{lccc}
    \toprule
    Generator & Self-Judge Faith. & Cross-Judge Faith. & Inflation \\
    \midrule
    Llama 3.3 70B & 0.929 & 0.500 & +0.429 \\
    Mistral Small 24B & 0.857 & 0.643 & +0.214 \\
    GPT-4o & 0.964 & 0.857 & +0.107 \\
    \bottomrule
  \end{tabular}
\end{table}

The bias is largest for Llama (+0.429), suggesting it is most susceptible to favoring its own output style. These results validate our decision to use Claude Sonnet 4 as an independent judge family for all evaluations.
...
To quantify LLM judge variance, we ran the full evaluation 3 times in parallel (GPT-4o + rerank, top\_k=10, 89 questions, Claude Sonnet 4 judge). Table~\ref{tab:multirun} shows the results.

\begin{table}[t]
  \caption{Multi-run averaging results (3 runs, GPT-4o + rerank, top\_k=10, 89 questions). Retrieval is fully deterministic; generation variance comes from LLM non-determinism. The 95\% CI is tight enough to distinguish model performance.}
  \label{tab:multirun}
  \centering
  \begin{tabular}{lccc}
    \toprule
...
Retrieval is fully deterministic (std 0.004, from rounding only). Of the 89 questions, 22 (24.7\%) showed unstable generation correctness (std $>$ 0.1), all with near-zero retrieval variance---confirming that generation non-determinism, not retrieval instability, drives score variance. The 95\% CI [0.865, 0.918] is tight enough to distinguish GPT-4o from Mistral (0.798) and Llama (0.730) with statistical confidence.
...
We compared BGE-large-en-v1.5 (1024-dim) against the default all-MiniLM-L6-v2 (384-dim) on all 89 questions. Contrary to expectations from the literature \citep{wiratunga2024cbrrag}, the larger model performed slightly worse on all metrics (MRR $-$0.021, Recall@K $-$0.023, NDCG@K $-$0.028). We attribute this to training domain mismatch: neither model was trained on legal text, and the larger embedding space may not confer advantages without domain-specific fine-tuning.
...
\begin{ack}
This work was completed as part of CS6180: Generative AI at Northeastern University, Spring 2026, taught by Prof.\ Karl Ni. We thank the course staff for feedback on the evaluation methodology. All data was scraped from publicly available government and legal aid websites. This system provides legal \emph{information}, not legal \emph{advice}.
\end{ack}
...
\bibitem[Gupta et~al.(2024)]{gao2024ragsurvey}
...
\bibitem[Ajay~Mukund and Easwarakumar(2025)]{rag_legal_survey_2025}
...
\newblock \emph{ResearchGate}, 2025.

\bibitem[Ajay~Mukund and Easwarakumar(2025b)]{symmetry2025legal}
Ajay~Mukund~S and K.~S.~Easwarakumar.
...
Nirmalie Wiratunga, Ramitha Abeyratne, Lasal Jayawardena, Kyle Martin, and Stewart Massie.
